...
class Rob_body(Environment):
    crashed = False
    def __init__(self, env, init_pos=(0.0, 0.0, 90.0)):
        ...
        self.sleep_time = 1 / 60  # time between actions (for real-time plotting)
    ...
    def do(self, action):
        ...
        if any(line_segments_intersect(path, wall) for wall in self.env.walls):
            ...
            if self.plotting:
                ...
                plt.draw()
                plt.pause(1000000) # wait for user to close
    ...
    def plot_status(self, status):
        plt.plot([self.rob_x], [self.rob_y], status)
        plt.draw()
    def whisker(self):
